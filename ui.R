           ...
           tabsetPanel(id = "tabpan1",
                       ...
                       tabPanel("Pixels",
                                ...
                                sliderInput("pixc_nodes", "Nodes to Show",
                                            min = 1, max = 1, 
                                            value = 1, step = 1),
                                radioButtons("improve_geoloc", "Pixel Geolocation",
                                             choiceNames = c("Medium (PIXC)", 
                                                             "Well-done (PIXCVec), using vector-level smoothed heights"),
                                             choiceValues = c("med", "wd")),
                                p("Below is a slant-plane image (range and azimuth coordinates) ",
                                  "for the vicinity of the pixels plotted on the map. ",
                                  "Pixels not on the map (including all land pixels and any water",
                                  " pixels not assigned to a node) are shown in a lighter color."),
                                plotOutput("slantplane"),
                                checkboxGroupInput("pixc_columns_select", 
                                                   "Table Columns To Display",
                                                   choiceNames = c("PIXC", "PIXCVec"),
                                                   choiceValues = c("pixc", "pixcvec"),
                                                   selected = c("pixc", "pixcvec")),
                                radioButtons("pixc_color", "Color pixels by",
                                             c("classification", "custom (from table)"))
                                ),
                       tabPanel("Nodes",
